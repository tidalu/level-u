    ...
    <div className="reveal homeBanner pt-32 pb-12 rounded-b-3xl px-5 lg:px-16 relative overflow-hidden 3xl:mx-32">
      ...
      <div className="relative">
        {slides.map((slide, index) => (
          ...
          >
            <div className="relative rounded-2xl overflow-hidden  lg:min-h-[663.38px] lg:grid lg:grid-cols-12 bg-[#6cce40] mx-auto">
              ...
              <div
                className={`reveal relative col-span-12 lg:hidden ${index === 1 ? "cursor-pointer" : ""}`}
                onClick={index === 1 ? handleMerchClick : undefined}
                role={index === 1 ? "button" : undefined}
                tabIndex={index === 1 ? 0 : undefined}
                aria-label={index === 1 ? "Visit merch store" : undefined}
                ...
                {/* {index === 0 && (
                  <div className="h-[60%] px-3 mt-2 sm:px-6 sm:mt-4 w-full absolute top-0 left-0 flex flex-col">
                    <p className="self-start text-black text-center text-lg sm:text-2xl apply-font-o">
                      {slide.topPic?.map((item: string, i: number) => (
                    ...
                    <p className="self-end sm:mt-7 text-center text-black sm:text-2xl mt-4 text-lg apply-font-o">
                      {slide.bottomPic?.map((item: string, i: number) => (
                        <React.Fragment key={i}>
                          {item}
                          {i !== 1 && <br />}
                        </React.Fragment>
        ...
        <div className="flex justify-center mt-4 gap-2">
          <button
            className={`w-2 h-2 rounded-full ${currentSlide === 0 ? "bg-[#6cce40]" : "bg-gray-300"}`}
            onClick={() => setCurrentSlide(0)}
            aria-label="Show slide 1"
          />
          <button
            className={`w-2 h-2 rounded-full ${currentSlide === 1 ? "bg-[#6cce40]" : "bg-gray-300"}`}
      ...
      {showConfirmation && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl transform transition-all duration-300 animate-in fade-in slide-in-from-bottom-4">
            <h3 className="text-xl font-bold text-gray-900 mb-4 apply-font-o">Visit Merch Store</h3>
            <p className="text-gray-700 mb-6">Would you like to visit our merchandise store?</p>
